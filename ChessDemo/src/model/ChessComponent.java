...
//    private static final Dimension CHESSGRID_SIZE = new Dimension(1080 / 4 * 3 / 8, 1080 / 4 * 3 / 8);
    private static final Color[] BACKGROUND_COLORS = {Color.WHITE, Color.BLUE};
    /**
     * handle click event
     */
    ...
    protected void processMouseEvent(MouseEvent ss) {
        super.processMouseEvent(ss);

        if(ss.getID()==MouseEvent.MOUSE_ENTERED){
            Graphics g=getGraphics();
            ((Graphics2D) g).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.5f));
            g.setColor(Color.GREEN);
            g.fillRect(0,0,this.getWidth(),this.getHeight());
        }
        if(ss.getID()==MouseEvent.MOUSE_EXITED)
        {paintComponent(getGraphics());}

        if (ss.getID() == MouseEvent.MOUSE_PRESSED) {
...
//                if (gameController.getChessboard().getChessComponents()[i][j].equals(new KingChessComponent(ss, location, ChessColor.BLACK, clickController, 76))) {
//                    b++;
//                } else if (gameController.getChessboard().getChessComponents()[i][j].equals(new KingChessComponent(ss, location, ChessColor.WHITE, clickController, 76))) {
//                    w++;
//                }
//            }
//        }
...
//    }
    ...
    public ArrayList<ChessboardPoint> getCanMoveTo(ChessComponent[][] chessboard){
        ArrayList<ChessboardPoint> canMoveTo = new ArrayList<>();
        for(int i=0;i<8;i++){
            for(int j=0;j<8;j++){
                if(this.canMoveTo(chessboard,new ChessboardPoint(i,j))){
